...
def get_data(data_dir, flatten_audio=False, flatten_image=False, unsqueeze_channel=True, generate_sample=False, normalize_image=True, normalize_audio=True):
    """Get datasets for AVMNIST.
    ...
    train_set = trainlist[0:55000]
    val_set = trainlist[55000:60000]
    test_set = testlist
...
if __name__ == "__main__": 
    folder_path = '../data/avmnist'
    train_set, val_set, test_set = get_data(folder_path)

    train_loader = DataLoader(train_set, batch_size=4, num_workers=2)

    batch = next(iter(train_loader))
    print(f'Modality 1: {batch[0].shape}, Modality 2: {batch[1].shape}, Labels: {batch[2].shape}')
